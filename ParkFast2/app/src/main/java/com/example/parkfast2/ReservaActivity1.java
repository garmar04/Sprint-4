...
public class ReservaActivity1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        datePickerLlegada = findViewById(R.id.datePickerLlegada);
        timePickerLlegada = findViewById(R.id.timePickerLlegada);
        datePickerSalida = findViewById(R.id.datePickerSalida);
        timePickerSalida = findViewById(R.id.timePickerSalida);
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            ...
                            if (document.exists()) {
                                ...
                                if (!tarjetaActiva) {
                                    Toast.makeText(ReservaActivity1.this, "Necesitas comprar nuestra tarjeta para poder realizar reservas", Toast.LENGTH_LONG).show();
                                } else {
    ...
    private void confirmarReserva(String userId) {
        ...
        if (isValidReservation(yearLlegada, monthLlegada, dayOfMonthLlegada, hourLlegada, minuteLlegada,
            ...
            Reserva reserva = new Reserva("pendiente", fechaHoraLlegada, fechaHoraSalida, "Parking 1");
                    ...
                    .addOnSuccessListener(documentReference -> {
                        db.collection("usuarios").document(userId).update("reservaActiva", true)
                                .addOnSuccessListener(aVoid -> {
                                    Toast.makeText(ReservaActivity1.this, "Reserva realizada con éxito", Toast.LENGTH_LONG).show();
                                    // Enviar la notificación
                                    sendNotification(userId, "Parking 1", fechaHoraLlegada, fechaHoraSalida);
                                    Intent intent = new Intent(ReservaActivity1.this, ReservasActivity.class);
                                    startActivity(intent);
                                    finish();
                                })
                                .addOnFailureListener(e -> Toast.makeText(ReservaActivity1.this, "Error al actualizar el estado de la reserva.", Toast.LENGTH_LONG).show());
                    })
                    .addOnFailureListener(e -> Toast.makeText(ReservaActivity1.this, "Error al realizar la reserva: " + e.getMessage(), Toast.LENGTH_LONG).show());
        } else {
            Toast.makeText(ReservaActivity1.this, "La fecha de salida no puede ser antes de la fecha de llegada.", Toast.LENGTH_LONG).show();
        }
    }
